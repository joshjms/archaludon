...
A cycle simulator for Da Vinci / Ascend-like NPU architectures. You describe a
...
YAML (`specs/da-vinci.yaml`) or built with the fluent `Module.add_*` API.
...
`specs/da-vinci-2core.yaml` is the single-core spec plus one line:
...
```bash
.venv/bin/python examples/visualize_da_vinci.py
```
