...
The 2-core spec is `specs/da-vinci.yaml` with `count: 2` on the AICore module
(see `specs/da-vinci-2core.yaml`). Bumping that to `count: 20` would give an
...
def main() -> None:
    chip = load(Path(__file__).resolve().parents[1] / "specs" / "da-vinci-2core.yaml")
