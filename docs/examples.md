...
are what the scripts produce on `specs/da-vinci.yaml` (or the 2-core spec).
...
`f = d + e` on AICore1, using `specs/da-vinci-2core.yaml`.
...
## visualize_da_vinci.py

Renders `specs/da-vinci.yaml` to Graphviz (`examples/da_vinci.dot`, plus a `.png`
