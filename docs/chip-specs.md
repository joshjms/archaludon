...
A chip spec is a YAML mapping describing a `Module` tree. `specs/da-vinci.yaml` is
the reference single-core chip; `specs/da-vinci-2core.yaml` is the same chip with
...
chip = load("specs/da-vinci.yaml")   # from a file
